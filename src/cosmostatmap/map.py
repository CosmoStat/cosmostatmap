...
class CosmoStatMap:
    ...
    def __init__(
        self,
        file_name,
        default_colour="#818181",
        current_colour="#0092CF",
        former_colour="#dbb578",
        water_colour="#505050",
        border_colour="#7a7a7a",
        map_projection=3395,
        hide_antartica=True,
        figsize=(12, 8),
    ):
    ...
    def _highlight_country(self, country, colour):
        ...
        """
        name = self.world[self.world.name == country].__geo_interface__["features"]
        namig0 = {
            "type": name[0]["geometry"]["type"],
            "coordinates": name[0]["geometry"]["coordinates"],
        }
        self._axis.add_patch(
            PolygonPatch(
                namig0, fc=colour, ec=self._border_colour, alpha=0.85, zorder=2
            )
        )
    ...
    def _make_plot(self, **kwargs):
        ...
        self.world.plot(
            ax=self._axis,
            color=self._default_colour,
            edgecolor=self._border_colour,
            **kwargs,
        )

        # Highlight countries in map
        self._add_country_from_list(
            self.countries_dict["current"], self._current_colour
        )
        self._add_country_from_list(self.countries_dict["former"], self._former_colour)
        ...
        custom_lines = [
            Line2D([0], [0], color=self._current_colour, lw=5),
